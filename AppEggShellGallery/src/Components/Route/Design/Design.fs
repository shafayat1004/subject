[<AutoOpen>]
module AppEggShellGallery.Components.Route_Design
...
type Ui.Route with
    [<Component>]
    static member Design(pstoreKey: string, item: DesignItem) : ReactElement =
        element {
                ...
                children = [|
                    LC.Section.Padded(
                        children = [|
                            ...
                            | DesignItem.ColorVariants -> Ui.Content.ColorVariants()
                            | DesignItem.Icons         -> Ui.Content.Icons()
                        |]
                    )
                |]
            )
