[<AutoOpen>]
module AppEggShellGallery.Components.Content_Input_PositiveDecimal
...
type Ui.Content.Input with
    [<Component>]
    static member PositiveDecimal () : ReactElement =
        Ui.ComponentContent (
                        ...
                        samples = (
                            element {
                                Ui.ComponentSample(
                                    ...
                                    code    = ComponentSample.SingleBlock (ComponentSample.Fsharp, LC.Text """LC.Input.PositiveDecimal(label = "Price", value = value, validity = Valid, onChange = setValue)
...
// empty -> Ok None (no internal error)""")
                                ...
                                Ui.ComponentSample(
                                    heading = "Zero (not positive)",
                                    visuals = Helpers.Sample(initialText = "0", validity = Valid),
                                    code    = ComponentSample.SingleBlock (ComponentSample.Fsharp, LC.Text """LC.Input.PositiveDecimal(label = "Price", value = value, validity = Valid, onChange = setValue)
// "0" -> internal error: Allowed only positive numbers""")
                                )
                            }
                        )
                    )
