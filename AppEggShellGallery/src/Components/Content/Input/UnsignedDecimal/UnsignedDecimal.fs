[<AutoOpen>]
module AppEggShellGallery.Components.Content_Input_UnsignedDecimal
...
type Ui.Content.Input with
    [<Component>]
    static member UnsignedDecimal () : ReactElement =
        Ui.ComponentContent (
            displayName = "Input.UnsignedDecimal",
            props       = ComponentContent.ForFullyQualifiedName "LibClient.Components.Input.UnsignedDecimal",
            a11y =
                Ui.A11yPanel(
                    componentName  = "LC.Input.UnsignedDecimal",
                        ...
                        samples = (
                            element {
                                Ui.ComponentSample(
                                    ...
                                    code    = ComponentSample.SingleBlock (ComponentSample.Fsharp, LC.Text """LC.Input.UnsignedDecimal(label = "Price", value = value, validity = Valid, onChange = setValue)
// "thirteen" -> internal error: Only numbers and period allowed""")
                                ...
                                Ui.ComponentSample(
                                    ...
                                    code    = ComponentSample.SingleBlock (ComponentSample.Fsharp, LC.Text """LC.Input.UnsignedDecimal(label = "Price", value = value, validity = Valid, onChange = setValue)
// "-13" -> internal error: Only numbers and period allowed""")
                                )
