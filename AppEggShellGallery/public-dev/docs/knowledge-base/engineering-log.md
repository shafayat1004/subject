...
## 2026-08-09 (session 52 — gallery SegmentedControl red-gate fix + verifier cache-clear gap)

Local `shafayat/neomorphism-fun` was 1 commit behind `origin` (missing PR #29 agent-os merge, `05de004`); fast-forwarded first (no path collisions with pre-existing untracked `.agent-os/`/`.mcp.json`/`evidence/` — those were only stray `__pycache__`/local files with no source overlap).

**Fix:** `AppEggShellGallery/src/Components/Content/SegmentedControl/SegmentedControl.fs` failed the compile verifier (`error FS764`, missing `ThumbShadowColor`) since `LibClient`'s `SegmentedControl.Theme` record gained `ThumbShadowColor` + `ThumbHighlightColor` fields (neumorphic raised-thumb bevel, session 50/51) but the gallery's demo `theme = (fun _ -> { ... })` literal — which fully replaces rather than updates the base theme — was never updated (rule-10 gallery-mirror drift). Added both fields with `Color.Transparent` (matches the app-wide default in `DefaultComponentsTheme.fs` and the type's own "Default Transparent = flat thumb" comment). Verified with a full cache-clear (`.build/web/fable` cleared repo-wide, not just the gallery's own) then `eggshell test-build`: two fresh "Started Fable compilation" runs, zero `error FS`.

**Gotcha found, not fixed (separate, pre-existing issue):** `policies/verification.yaml`'s `compile` verifier only clears `AppEggShellGallery/.build/web/fable` before running `eggshell test-build`, but that command also (re)compiles dependency libs (`LibStandard`, `LibRouter`, ...) via their own `.build/web/fable` caches. Any prior successful build leaves those caches warm, so the very next verify run prints `Skipped compilation because all generated files are up-to-date!` for a dependency lib even though the app itself did a fresh, correct compile — tripping the verifier's `excludes: "Skipped compilation..."` assert as a false RED. `agentos verify` reported `compile: fail` on this repo state even though the actual SegmentedControl bug was fixed and a manually-full-cache-cleared build was clean. Did not change `policies/verification.yaml` (out of scope for this task, and not asked); flagging here so the next session does not treat `agentos verify compile: fail` alone as proof this fix regressed — check for `error FS`/`ThumbShadowColor` specifically, not just the gate's pass/fail bit.
...
CLAUDE.md rule was reactive scar tissue, and the only zero-recurrence discipline was the one fully
tool-enforced one (eggshell-fmt). The recurring, costliest failures were all prose-enforced: false
greens / done-without-evidence, forbidden-path edits, stale-memory action, a11y skips, docs drift.
Industry research (codemem 1920: Anthropic hooks/best-practices, 12-factor agents, guardrails-as-code,
Chroma context-rot) says the same thing: models advise, deterministic processes gate.

Implemented the overhaul (all uncommitted, on this branch):

- **Gates** (`scripts/`): `verify-done.sh` (done-gate: fails on `error FS` / "Skipped compilation"
  stale-cache logs, requires fresh-compile evidence), `forbidden-path-guard.sh` (blocks
  `_autogenerated_`/`*.render`/`*.typext.fs`/stray `.fs.js`, warns on `App*`/`Suite*`),
  `install-git-hooks.sh` (pre-commit: path guard + `eggshell-fmt --check` on staged .fs; installed),
  `ci-gate.sh` (one-shot: path guard + fmt + style-leak + a11y; future CI entry),
  `symptom-match.sh` (error string/log -> ranked runbook matches), `status-dashboard.sh` (generated
  modernization numbers; verified live: 0 `.render`, 176 `[<Component>]` files, 79 gallery pages).
- **Hooks**: `.claude/settings.json` PreToolUse on Edit|Write (path guard) and on `dotnet fable`
  (blocks direct invocation). Claude Code honors them; other harnesses fall back to the git hook.
- **New skills**: `symptom-matcher` (run symptom-match.sh before improvising root cause),
  `orleans-test-triage` (stasis/timeout/CodecNotFound triage), `scaffolding-validator` (BLOCKED
  pending Goal B; exits 3, no fake pass).
- **New docs**: `fsharp/interop-invariants.md` (7 rules for F# at grain/JS/worklet/sourcegen
  boundaries: closures never cross, box lambdas at JS registration, C# sourcegen misunderstands F#
  codegen, client-side codec audit, Fable.Core 5 namespace moves, auto-open limits, SRTP fragility),
  `runbooks/backend.md` (dev-stack modes, silo boot failure catalog, Orleans test triage). Wired into
  runbooks/index.md, maintaining-docs.md, llms.txt.
- **CLAUDE.md diet + corrections**: Goal A marked DONE (zero `.render` in framework libs; 2 LibRouter
  `.typext.fs` stragglers remain), conversion recipe rules collapsed to a pointer, added the
  "Deterministic gates" section, added the codemem trust policy (act on memory, verify on failure,
  repair stale entries same-session), skills index completed.

**User correction that changed the design:** codemem must stay a trusted token-saving layer. The
S48 dev-stack failure was trusting memory to avoid running the tool at all, not trusting memory per
se. So the policy is read-side trust + write-side hygiene: verify on-disk ONLY after an action fails,
and any stale entry discovered in practice gets repaired in the same session. Preemptive re-reads
would burn the tokens memory exists to save (context-rot data: ~30% degradation at high fill).

**Distilled:** enforcement must live outside the model's context (hooks, linters, CI); prose rules
either graduate to scripts or get culled. Memory is L2 cache: trust it, repair it, never preemptively
re-verify it. Done-claims require artifacts, not exit codes.
...
**What shipped:**
- `release-build` skill: new `scripts/ios-preflight.sh` (10 deterministic checks, PASS/FAIL/WARN with the exact value tested, exits non-zero with an actionable fix on any hard FAIL) and a rewritten iOS branch in `scripts/release-build.sh` that runs the preflight, picks the newest `.xcarchive` for the app, verifies it is signed + has `main.jsbundle` + `embedded.mobileprovision`, and wraps the already-signed `.app` into `<bundleid>.ipa` under `<appdir>/dist/ios/`. `SKILL.md` documents the three-stage flow (preflight -> archive in Xcode UI -> wrap) and the iOS release-only failure modes.
- Podfiles (AppTodo + Gallery) `post_install`: loop setting `CODE_SIGNING_ALLOWED=NO` / `CODE_SIGNING_REQUIRED=NO` / `CODE_SIGN_IDENTITY=''` on every pod target's build configurations, then `pod install`. Pod targets no longer try to sign with the legacy `iOS Development` identity.
- iOS runbook: new "Release archive and IPA export" section. Troubleshooting: new "iOS signing and IPA export" table.

**The four gotches, now encoded so they are not rediscovered:**
1. **Pod-target signing.** Pods inherit the manual-signing `iOS Development` identity that no modern `Apple Development` cert matches, so archive fails on every RN pod ("No signing certificate 'iOS Development' found"). Pods never need their own signing -- the app target re-signs embedded frameworks at `[CP] Embed Pods Frameworks`. Fix is the Podfile loop above. Preflight check 7 verifies it.
2. **Team / cert / account alignment.** Three things must agree: the keychain `Apple Development` cert (team id is the parenthesized suffix), the team logged into Xcode -> Settings -> Accounts, and `DEVELOPMENT_TEAM` in the pbxproj. A free personal team cannot have its signing cert generated by `xcodebuild -allowProvisioningUpdates` (profiles yes, cert no) -- the first archive MUST be in the Xcode UI, which generates the cert and holds the login-keychain unlock. Preflight checks 4-6 verify all three.
3. **`errSecInternalComponent` from the CLI.** `xcodebuild archive` and `xcodebuild -exportArchive` re-sign embedded frameworks via `codesign`, which cannot unlock the login keychain non-interactively. Archive via the Xcode UI; for the IPA, wrap the signed `.app` from the `.xcarchive` into `Payload/` + `zip` (a Development IPA is just that) -- do NOT use `-exportArchive`, which re-signs and hits the same wall. This also bypasses the Xcode "release testing" gate (TestFlight/App Store, needs a $99 paid program), which is not required for own-device Development install.
4. **Config + bundle freshness.** AppTodo `index.js` loads `configSourceOverrides.native.js` (dev, `BackendUrl=http://localhost:5000`) unconditionally, unlike Gallery which gates on `__DEV__`; a Release AppTodo ship would carry dev config. Separately, the Release "Bundle React Native code and images" phase bundles from `.build/native/commonjs`, so a stale Fable native build ships old JS. Preflight checks 8-9 flag both.

**Verification:** preflight runs green on AppEggShellGallery (11 PASS / 1 WARN) and on AppTodo (9 PASS / 2 WARN / 1 FAIL until `eggshell build-native` re-ran; then fresh). Both `.xcarchive`s (Xcode UI archives) verified signed with `main.jsbundle` + profile; wrapped into `AppEggShellGallery.ipa` (16M) and `AppTodo.ipa` (8.6M) via the new script's manual wrap. Note: AppTodo's first archive was done before `build-native`, so its IPA had a stale bundle; `build-native` was re-run and AppTodo must be re-archived to ship fresh JS.

**Also fixed:** zsh does not glob the RHS of an assignment by default, so the preflight resolves paths via `ls -d` (and excludes `Pods.xcodeproj` when finding the app's pbxproj). Scheme detection prefers the scheme matching the app/workspace stem -- `xcodebuild -list` prints schemes alphabetically, so the first is often a pod like `AppCenter`, not the app scheme.
...
- **agent-os enforcement audit (2026-08-09) — four false-green custom checks + unsatisfiable compile verifier** — `agentos all` reported all custom checks PASS, but direct execution proved 4 of 5 were silently passing on empty input. Root cause was a single bug in the vendored `.agent-os/agentos/yaml_min.py` double-quote escaper: it over-escapes `\"` to `\\"` (literal backslash-quote), so any inlined `git ls-files "*.fs"` glob or `printf "\xe2\x80\x94"` byte literal matched nothing. em-dash-scan (0 -> 67 files), eggshell-fmt-check (0 -> 7 editable files), a11y-scan, and style-leak-scan all false-greened; doc-links and fmt-check were *additionally* swallowed by a trailing `|| exit 0` that masked the real scanner exit code even where the glob survived. Fix (no agent-os source changes): moved the byte/glob-sensitive enumeration into `scripts/em-dash-scan.sh` and `scripts/run-scanner.sh`, so the YAML commands are plain script paths with no nested quotes; restructured fmt/doc-links/a11y/style-leak to `if tool-present; then run; else exit 0; fi` so the fail-open fires ONLY on a missing tool, not on a real violation. The compile verifier was separately broken: it ran `./eggshell build-lib` from the repo root, but the root `eggshell.json` is `type: repoRoot` (buildLib rejects non-library), and `buildLib` never calls `runFable` at all — so the `contains: "Started Fable compilation"` assert was *structurally unsatisfiable* regardless of cwd. Fixed to `cd AppEggShellGallery && rm -rf .build/web/fable && ../eggshell test-build` (app project -> testBuild -> runFable -> Fable CLI emits the marker). The verifier now correctly fails on the real `SegmentedControl.fs` `ThumbShadowColor` FSHARP error in the tree. Takeaway: any agent-os custom check that needs a quoted glob or byte literal in its YAML command must delegate to a `scripts/*.sh` wrapper until yaml_min's over-escape is fixed upstream.

- **agent-os v0.7.1 upgrade (2026-08-09) — upstream hook-pre-tool stdin fix landed, local workaround reverted** — Upstream agent-os v0.7.1 added the adapter-mode guard to `hook-pre-tool` (cli.py:178-198), the same pattern as codemem 1978's `hook-post-tool` fix: when `--tool`/`--file-path`/`--command` flags are given, stdin is never read. This eliminates the latent never-EOF stdin hang (1978 note #3) for any harness that passes flags. The local workaround (`scripts/hook-pre-tool.py`, a select()-based non-blocking stdin reader that called `check-path`/`check-command` subcommands) is no longer needed and was removed; `.claude/hooks/agentos-pre-tool` reverted to the upstream init template (stdin path, safe under Claude Code which pipes+closes stdin). Verified: `hook-pre-tool --tool bash --command "dotnet fable"` with a never-EOF stdin returns promptly exit 2 (no hang), and all 8 Claude Code PreToolUse enforcement cases pass. The opencode plugin already used the flag-based `check-path`/`check-command` path, so both harnesses are now covered by upstream code.

- **old agentic-system artifact sweep (2026-08-09) — deleted 6 obsolete scripts, .cursor/, ~/.claude-mem** — After agent-os took over enforcement, six `scripts/` files were dead code with zero live references: `hook-bash-guard.sh` (old PreToolUse `dotnet fable` block, replaced by `command-policy.yaml`), `hook-edit-guard.sh` (old PreToolUse path block, replaced by `path-policy.yaml`), `install-git-hooks.sh` (old `.git/hooks/` installer, replaced by `agentos init` + `core.hooksPath=.githooks`), `forbidden-path-guard.sh` (called only by the two deleted hooks), `ci-gate.sh` (aggregated CI gate, replaced by `agentos all` + `.githooks/pre-commit`), `verify-done.sh` (Stop-gate build-log grep, replaced by `agentos done` -> `agentos verify`). Surviving scripts: `em-dash-scan.sh`, `run-scanner.sh` (agent-os custom checks), `symptom-match.sh`, `status-dashboard.sh` (AGENTS.md), `validate-scaffold.sh` (Goal B stub). Stale `ci-gate.sh` references in `custom-checks.yaml` line 8 and `.githooks/pre-commit` line 11 were comments only (not calls) and were cleaned; both files now reference AGENTS.md instead of CLAUDE.md. `.cursor/rules/` (4 Cursor editor rule files) was deleted (duplicated by AGENTS.md, Cursor no longer used). `~/.claude-mem/` (22MB local db + logs + observer sessions, superseded by codemem per the `codemem-update` skill: "claude-mem auto-capture is retired") was `rm -rf`'d locally (not a git artifact). `CLAUDE.md` (3-line redirector to AGENTS.md) was KEPT because Claude Code auto-reads it as the entry point. `agentos all` + `agentos doctor` both pass after the sweep.

- **AppTodo native haptics landed (2026-08-09) -- root cause was RN 0.86's prebuilt-React checksum pin, not the haptics library** -- Two earlier sessions tried wiring `react-native-haptic-feedback` and hit a Fabric link break (`undefined facebook::react::Sealable`/`DebugStringConvertible`, missing `UIUtilities`) and gave up, shipping a no-op `Feedback.fs` that was never committed and is now gone from the tree entirely (confirmed via `git log --all`, zero history). GitHub research (facebook/react-native#55446 "Inconsistent Podfile.lock checksums for ReactNativeDependencies and React-Core-prebuilt"; invertase/react-native-firebase#8883) confirmed this is a known RN 0.80+ issue: `RCT_USE_PREBUILT_RNCORE`/`RCT_USE_RN_DEP` default to `'1'` in `react_native_pods.rb`, pinning a prebuilt `ReactNativeDependencies.xcframework` by checksum; any `pod install` that adds a new pod can re-resolve `React-Core` past that pin and break the prebuilt binary's ABI compatibility -- nothing to do with `react-native-haptic-feedback` itself (confirmed at latest 3.0.0, which already has proper New Arch/Fabric turbomodule support). Fix: `SuiteTodo/AppTodo/ios/Podfile` now sets `ENV['RCT_USE_PREBUILT_RNCORE'] = '0'` and `ENV['RCT_USE_RN_DEP'] = '0'` before `prepare_react_native_project!`, forcing a from-source `React-Core` so pod resolution can never drift off a checksum pin again (documented upstream workaround). `pod install` removed `React-Core-prebuilt`/`ReactNativeDependencies` and installed `RNReactNativeHapticFeedback (3.0.0)` alongside ~90 source-built React pods (37s incremental; first from-source build is the slow one). Recreated `SuiteTodo/AppTodo/src/Feedback.fs` (`buttonTap`/`toggleSwitch`, Taptic `impactMedium`/`rigid`, `#if EGGSHELL_PLATFORM_IS_WEB` no-op guard matching the LibClient convention) and wired it into `Todos.fs`'s `CategoryPill` (`onPress`) and `ThemeToggle` (`onSelect`). Verified with a genuinely clean build (wiped `DerivedData` first, per the earlier lesson that incremental builds mask a broken link): `xcodebuild ... clean build` on Release -> `** BUILD SUCCEEDED **`, zero `Sealable`/undefined-symbol errors.
