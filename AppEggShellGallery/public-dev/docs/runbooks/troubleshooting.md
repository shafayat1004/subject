...
| Native red-box at app root: *"Element type is invalid ... got `<GestureHandlerRootView />`. Did you accidentally export a JSX literal?"* after a **clean** `rm -rf .build/native/fable` + `build-native` | Pre-existing Fable-5 / RNGH-3 / Metro interop bug: a fresh Fable output makes `react-native-gesture-handler`'s `GestureHandlerRootView` resolve to a React **element** instead of a component (reproduces on the pre-RW2 baseline). A stale/incremental `.build` renders past root; a clean one does not | **Open** -- blocks native runtime for AppTodo + gallery. Tracked as RW7 in [RN 0.86 status](./modernization/rn86-upgrade-status.md). Candidate: how Fable's `import "GestureHandlerRootView"` interops with RNGH 3's `export { default as GestureHandlerRootView }` under Metro. |
