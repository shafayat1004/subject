...
| **Web** app boots to `Loading…` then dies; browser console `ReferenceError: process is not defined` at `react-native-worklets/lib/module/platformChecker.js` | worklets reads `process` at import time; the **web** webpack bundle has no `process` shim (a polyfill gap in the worklets modernization tail, unrelated to app code). Blocks `npm run audit:web` ("Todo UI not ready") | Not yet fixed. Provide `process` in the web build (webpack `DefinePlugin`/`ProvidePlugin` or a `platformChecker` alias/shim) alongside the Reanimated-4 / worklets work. Formatting/whitespace changes are NOT a cause -- the failing file is in `node_modules`. |
