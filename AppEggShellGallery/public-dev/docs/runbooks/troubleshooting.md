...
| `TimeSpan.FromSeconds/FromMinutes/FromMilliseconds` -> `FS0041` "unique overload ... could not be determined" (net8+) | net8 added `int64` overloads, so an `int` arg is ambiguous (int64 vs float) | Make the arg `float`: literals `1` -> `1.0`; variables/exprs -> `float x`. For a point-free method group (`Gen.map TimeSpan.FromMinutes`), use a lambda `(fun m -> TimeSpan.FromMinutes (float m))`. |
| `NU1510` "PackageReference X will not be pruned ... likely unnecessary" as a restore error (WarningsAsErrors) | net10 prunes framework-provided packages; the ref is redundant | Remove the redundant `PackageReference` (e.g. `System.Text.Json`) if the code doesn't need a specific version -- the framework provides it. |
| Fable build fails compiling `fable_modules/FSharpPlus.1.6.1/Extensions/Extensions.fs` -> `FS0957`/`FS0341` on `IEnumerable<'T>` type extension (`allows ref struct`) | .NET 9 gave BCL `IEnumerable<'T>` an `allows ref struct` constraint; FSharpPlus's `GetSlice` type extension can't declare it (no F# syntax; dotnet/fsharp#18001). Fable source-compiles FSharpPlus against the **running SDK's** net10 refs -- **independent of the project TFM** (lowering a project to net8 does NOT change the refs Fable uses) | Patch the FSharpPlus fable source to drop the unused extension. Done in `Meta/LibFablePlus/src/index.ts` `patchFSharpPlusExtensionsForNet9()` (auto-applied every Fable run; sibling of `patchFSharpPlusComonadForFable5`). Do NOT try a per-project net8/multi-target split -- Fable ignores the TFM for BCL refs. |
| `eggshell test-build` "succeeds" (exit 0) but the frontend is actually broken | `test-build` **false-greens** -- the wrapper's exit code does not reflect Fable/webpack failure | Judge by the log, not the exit code: grep for `error!`, `error FS`, `code 957`, `Compilation failed`, `precompilation failed`. Green only if none are present and you see `completed!` / webpack `compiled`. |
| dev-web dies on `FSharpPlus.1.6.1/.../Extensions.fs code 957` **even though the fix is committed** | `Meta/LibFablePlus/dist/` is gitignored, so a checkout can carry a stale `dist/index.js` predating `patchFSharpPlusExtensionsForNet9` (its `src/index.ts` is newer) | Rebuild the patch tool: `cd Meta/LibFablePlus && ./initialize` (runs `npm run build` -> `tsc`). Verify: marker `EGGSHELL_NET9_IENUMERABLE_SLICE_PATCH` present in `~/.nuget/packages/fsharpplus/1.6.1/fable/Extensions/Extensions.fs` after the next Fable run. |
...
| Web console: `<h1> cannot be a child of <h1>` / `<h1> cannot contain a nested <h1>` (hydration error) | A container View had `accessibilityRole = Header`; RN `Header` -> ARIA `heading` -> RNW renders `<h1>`, and a child (e.g. `LC.Nav.Top.Heading` title) is also an `<h1>` | Remove `accessibilityRole = Header` from the container. Put the heading role on the actual title text only; let the nav *landmark* (`role=navigation` + `accessibilityLabel`) come from the wrapping `<nav>`. (Was `Nav.Top.Base`; the bar is a plain box, the title is the heading.) |
...
| `eggshell test-build` reports success (exit 0) but the build actually failed | `test-build` prints `Fable precompilation failed` / `error! 1` yet exits 0 | Do NOT trust its exit code. Grep the log for `error` / `failed` (excluding `removing build`); treat any hit as failure. |
| `error FS0010: Unexpected keyword 'static'/'interface'` right after running `eggshell-fmt` | Brace normalization wrongly reflowed a record TYPE that has members into the `type X = {` ... `}` form; the col-0 `}` ends the `type` block (offside), orphaning the trailing member. Fixed in eggshell-fmt 1.6.3 | Update the tool (`./Meta/EggShellFmt/install.sh`). By hand: keep the indented-brace form (`{ ... }` under `type X =`) for any record type with members. |
| `eggshell-fmt --check .` wants to reformat ~hundreds of `fable_modules`/vendored files | `.build/<platform>/fable` (copied third-party source) is not skipped by default | Ensure `**/.build/` is in `.eggshellfmtignore` (added session 19). Never reformat generated Fable output. |
...
| Web console warning: `"shadow*" style props are deprecated. Use "boxShadow".` | The `shadow` helper emitted `shadowColor`/`shadowOffset`/`shadowOpacity`/`shadowRadius`; react-native-web deprecates these on web | `shadow` now emits a single `boxShadow` CSS string on web (`#if EGGSHELL_PLATFORM_IS_WEB`: `sprintf "%dpx %dpx %dpx %s" offsetX offsetY blur color.ToCssString`); native keeps the shadow* quartet + Android elevation. A `boxShadow` rule was added to New + Legacy `RulesBasic`. |
...
| Web console error: `BackHandler is not supported on web and should not be used` (stack: `NativeBackButton.js` -> `EggShellReact.js` useEffect) | `LR.NativeBackButton` called `BackHandler.addEventListener("hardwareBackPress", ...)` unconditionally; react-native-web's `BackHandler` is a stub that logs a console error on every `addEventListener` | Guard the effect body (and cleanup) with `if not (Rn.Runtime.isWeb ()) then ...` (runtime guard, matches `LR.EdgeSwipeBack`); keep the `useEffectDisposableFn` hook itself unconditional (Rules of Hooks). iOS is unaffected (BackHandler never fires there). |
...
| `[<Component>]`/memo'd FC still logs "Each child in a list should have a unique key prop" even though its return went through `castAsElementAckingKeysWarning` / `Children.toArray` | A bare (even `toArray`-keyed) array RETURNED FROM a function component is reconciled as the FC's own output and React 19 warns for it; `toArray` keys do not save it. (`element { ... }` CE / `asFragment` wrap in a Fragment so they don't warn; `castAsElementAckingKeysWarning` used to return a bare array) | `castAsElementAckingKeysWarning` now wraps in a `Fragment` (`fragment [] (tellReactArrayKeysAreOkay els)`), matching `ElementBuilder.Run`. If you write a new helper that returns an array from an FC, wrap it in a Fragment (or `element { ... }`), not a bare `!!`-cast array. |
...
| **Web** app boots to `Loading…` then dies; browser console `ReferenceError: process is not defined` (`react-native-worklets/.../platformChecker.js`) or `ReferenceError: __DEV__ is not defined` (`react-native-reanimated`) | Webpack 5 does not inject the `process`/`__DEV__` globals the reanimated+worklets stack reads at import time; blocks boot (`audit:web` -> "Todo UI not ready") | **Fixed**: `Meta/LibFablePlus/webpack.config.js` `DefinePlugin` defines `process.env.JEST_WORKER_ID` and `__DEV__` for every app's web bundle (dev + prod). Formatting/whitespace changes are NOT a cause -- the failing files are in `node_modules`. |
| **iOS** an invisible-overlay control does nothing on tap: the **Picker/dropdown does not open**, or an `Input.Text`/`Duration`/`LocalTime` field is unclickable (works on web + Android). `onPress` never fires; nothing in the LogBox | The control makes its whole surface tappable with an invisible overlay `LC.Pressable` (`overlay=true`) styled `opacity 0`. On the New Architecture (Fabric) iOS, `RCTViewComponentView.betterHitTest` treats any view with **`alpha < 0.01` as non-interactive** (react-native [#50465](https://github.com/facebook/react-native/issues/50465)) -- a breaking change vs Paper, which hit-tested opacity-0 views. So every invisible-overlay pressable went dead on iOS after the Fabric move. | Style the invisible overlay with **`opacity 0.02`, never `0` (and not even `0.01`, which sits exactly on the dead threshold)**. Visually identical for an empty, background-less overlay; above the hit-test floor. Fixed framework-wide in the five `pressableOverlay` styles (`PickerInternals/Field`, `PickerInternals/Dialog`, `Input/Text`, `Input/Duration`, `Input/LocalTime`). Any new invisible tap target must clear `alpha >= 0.02` (or make the visible content itself the Pressable). JS-only change. Log 2026-07-15 (session 21). |
| **iOS** red-box at launch: *"Failed to call into JavaScript module method RCTEventEmitter.receiveEvent(). Module has not been registered as callable. Registered callable JavaScript modules (n = 7): …"* (bridgeless/Fabric on; only ~7 callable modules; app never renders). Often preceded by *"Unimplemented component: `<RNCSafeAreaProvider>`"* | The iOS `AppDelegate` never sets `self.dependencyProvider`. Under RN 0.77+ New Arch, `RCTAppDependencyProvider` is what wires the autolinked `thirdPartyFabricComponents` (incl. `RNCSafeAreaProvider`) + TurboModules into the Fabric registry. Without it, `SafeAreaProvider` has no Fabric descriptor, renders via the **legacy interop** path, and its mount-time `onInsetsChange` dispatches through legacy `RCTEventEmitter.receiveEvent`, which bridgeless never registers. (Android is unaffected -- its `MainApplication` registration path already wires this.) | Add to `ios/<App>/AppDelegate.mm`: `#import <ReactAppDependencyProvider/RCTAppDependencyProvider.h>` and, in `didFinishLaunchingWithOptions` before `[super …]`, `self.dependencyProvider = [RCTAppDependencyProvider new];`. Rebuild native (`run-ios`). Verify: no `receiveEvent` line on the `com.facebook.react.log:javascript` channel and the app renders with insets applied. Add this line to the `eggshell create-app` iOS scaffold. Log 2026-07-15 (session 21). |
...
| Symptom | Cause | Fix |
|---|---|---|
| Archive fails on every RN pod: *"No signing certificate 'iOS Development' found ... matching team ID ..."* (RNGestureHandler, RNSVG, RNReanimated, …) | Pod targets inherit the legacy `iOS Development` identity that no modern `Apple Development` cert matches; they were never meant to sign themselves | Add to the Podfile `post_install` a loop over `installer.pods_project.targets` setting `CODE_SIGNING_ALLOWED=NO`, `CODE_SIGNING_REQUIRED=NO`, `CODE_SIGN_IDENTITY=''` on every build configuration, then `pod install`. The app target re-signs embedded frameworks at `[CP] Embed Pods Frameworks`; pods never need their own signing. (Applied to AppTodo + Gallery.) |
| Archive fails: *"No Account for Team XXXXXXXXXX"* or *"No profiles for 'com.eggshell.X' were found ... Automatic signing is disabled and unable to generate a profile"* | `DEVELOPMENT_TEAM` in the pbxproj is not logged into Xcode -> Settings -> Accounts (or the bundle id is registered to a different team) | In Xcode add the Apple ID that owns the team; ensure the workspace's Signing & Capabilities uses that team. The preflight (check 5) verifies the team is logged in before you archive. |
| `xcodebuild archive` or `xcodebuild -exportArchive` fails `errSecInternalComponent` during `codesign` of a `.framework` (React.framework, FBAEMKit, …) | The CLI cannot unlock the login keychain non-interactively to access the signing identity's private key | Archive via the Xcode UI (its session holds the keychain unlock). For the IPA, wrap the already-signed `.app` from the `.xcarchive` into `Payload/` + `zip` (a Development IPA is just that); do NOT use `xcodebuild -exportArchive`, which re-signs and hits the same wall. |
| A free personal team: `-allowProvisioningUpdates` fetched a profile but there is still no `Apple Development` cert in keychain | `xcodebuild -allowProvisioningUpdates` generates profiles from the CLI but NOT a signing certificate for a free personal team; cert generation needs the Xcode UI session | The first archive MUST be done in Xcode UI (Product -> Archive); it generates the cert. Subsequent CLI archives then work once the cert exists and the keychain is unlocked. |
| Release app ships dev config (`BackendUrl=http://localhost:5000`, `InitializeRnInDevMode=true`) | `index.js` requires `configSourceOverrides.native.js` (the dev override) unconditionally; a Release build has `__DEV__` false but the require still ran | Gate on `__DEV__` like the gallery: `if (__DEV__) { require("./configSourceOverrides.native.js") } else { require("./configSourceOverrides.native.prod.js") }`. Preflight (check 9) warns on this. |
| Release `main.jsbundle` is old / missing recent source edits | The "Bundle React Native code and images" phase bundles from `.build/native/commonjs`; if the Fable native build predates `src/*.fs`, the archive ships stale JS | `(cd <appdir> && ../../eggshell build-native)` then re-archive. Preflight (check 8) compares Bootstrap.js mtime to the newest `src/*.fs`. |
| Confusion: the archived app's home-screen label does not match the app/dir name (e.g. AppTodo shows "Todo") | `CFBundleDisplayName` in `ios/<App>/Info.plist` is a cosmetic label, not a reference to another app or a wrong-project reference | Cosmetic only; edit `CFBundleDisplayName` to change it. Preflight (check 10) reports it so it is not mistaken for a misreference. |
| SideStore / LiveContainer refuses to install: *"the IPA does not exist"* even though the build succeeded | The IPA is valid, but it was installed from a scratch path (`/tmp/eggshell-ipa`, a `mktemp` dir); tmp is reaped on reboot / next build, so the sideloader's referenced file is gone. (The IPA is a well-formed `Payload/<App>.app` zip; `unzip -t` passes.) | Install only from `<appdir>/dist/ios/<bundleid>.ipa` (stable, in-repo). Delete stale scratch copies (`rm -rf /tmp/eggshell-ipa`). The `release-build.sh` iOS branch already writes the canonical IPA to `dist/ios/` and cleans its own `mktemp` scratch. |
| Two IPAs of one app with identical byte size but different `md5` | One came from `xcodebuild -exportArchive` (with an `exportOptions.plist`), the other from the current `Payload/`-wrap flow; same `.app` payload, different zip → same size can coincide but bytes differ | Use the `dist/ios/` one (the wrap flow). The `-exportArchive` path is abandoned (`errSecInternalComponent`); do not install its leftovers. |
