...
| `react-native-gesture-handler` Kotlin compile error `Cannot access 'ViewManagerWithGeneratedInterface'` on `run-android` | The installed RNGH major is too new for RN 0.76.5 (assumes newer RN codegen) | Pin **`react-native-gesture-handler@2.21.2`** (RN-0.76-era), then rebuild. |
