...
Related: [Dev loop](./dev-loop.md) | [Build and rebuild](./build-rebuild.md) | [Audit toolkit](./audit-toolkit.md) | [Troubleshooting](./troubleshooting.md)
...
| A **horizontal** `LC.ScrollView` (chip / tab row) clips content — pill tops cut, or the bottom edge is eaten where the scrollbar sits (Chrome + steady state) | RNW's ScrollView base style is `flexShrink: 1`, so in a column-flex parent the scroller is shrunk below its content height (collapse); the short box then clips content and a classic scrollbar sits over it. **Fixed in `LC.ScrollView`**: a horizontal scroller's outer gets `flexShrink 0` (height tracks content) and its content reserves a scrollbar-height bottom strip. (Do NOT hide the scrollbar to "fix" this — hiding it removes the scroll affordance.) | Confirm the scroller's computed `flex-shrink: 0`. Do not re-add per-app padding hacks; the behavior lives in the framework component. |
| Same horizontal `LC.ScrollView` still clips on **first mount in Safari only**, self-correcting on the next render | Safari.app lays out the RNW/Fabric horizontal ScrollView wrong on first paint (a browser quirk; our sizing is static). **NOT reproducible in tooling** — Playwright (Chromium + WebKit) always uses overlay scrollbars and renders it correct from first paint. A mount-time forced re-render did not help. | **BACKLOG** — see [rn86-upgrade-status](../modernization/rn86-upgrade-status.md) RW9. Likely fix: an explicit definite height on the horizontal scroller (framework prop) to remove the async first-paint layout. Verify on a real Safari. |

For a complete catalog of build, styling, and layout gotchas across all platforms, see [Troubleshooting](./troubleshooting.md).
