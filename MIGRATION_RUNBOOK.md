...
4. Established the animation layer (Reanimated 4 / RNGH 3 / Moti) F# wrappers - keep app code
   declarative. **Worklets are NOT authored in F#** (settled 2026-06-29, see §4.6 worklet rule): any
   genuine UI-thread worklet lives in a tiny vetted JS shim and is called from F#. Earlier notes that
   "they worked from F# directly" are superseded.
...
### 4.9 Per-component recipe (deterministic; the weaker model follows this verbatim)

This expands the §4-prose fan-out recipe into a copy-paste checklist. **Do ONE primitive at a time. Do
not start the next until the current is green on web + iOS + Android.** Run `export
DOTNET_ROOT="$HOME/.dotnet"` once per shell. Framework-only (`Lib*`, `Meta/*`); never touch `App*`/`Suite*`
beyond palette/theme; never patch `.render` or autogenerated `*.Render.fs`.

1. **Scope / STOP check.** If the component is `GestureView` or any `Animatable*`, STOP — that is the
   animation/gesture workstream (design work, strong model; §4.11 trigger B/E). Otherwise proceed.
2. **Read reference, then target.** Read `LibClient/src/ReactXP/Components/View/View.fs` (the pattern you
   imitate), `LibClient/src/ReactXP/RNSeam.fs` (the adapters you call), and the current `<Primitive>.fs`
   (note its import and *every* optional arg / DU / record type).
3. **Keep the F# signature byte-identical.** Do not rename, drop, or reorder any public param. Keep
   `?xLegacyStyles` (dropping it broke callers before), every
   `accessibility*`/`aria*`/`testId`/`liveRegion`/`importantForAccessibility` param, public DU/record
   paths, and themeable fields (`Input.Text.Theme`, `PickerInternals.Field.Theme`). If RN/RNW genuinely
   cannot express a param (RNW `Text` has no `onLongPress`; no `RefreshControl` on web), the param STAYS
   and is gated/no-op'd — never silently dropped (§4.11 trigger F).
4. **Repoint import + port styles.** Change `ReactXPRaw?X` / `@chaldal/reactxp*` to the RN target from the
   §4.6 mapping table. Route style construction through `RNSeam.createViewStyle` / `createTextStyle`
   (they replicate ReactXP flex-expansion + the numeric-`lineHeight`→`px` fix). Bridge `?xLegacyStyles`
   with the `__props?__style <- styles` pattern.
5. **Wire seam adapters in `View.fs` order**, before `createElement`:
   `assignPointerEvents` → `assignTestId` → `assignOnLayout` (MANDATORY if the primitive has `onLayout` —
   raw forwarding zeroes width/height) → `assignAccessibility` (role via
   `accessibilityRole |> Option.bind RNSeam.mapAccessibilityRole |> Option.map box`, **never** the raw int
   enum) → if pressable `assignPressableFeedback` → `RNSeam.createElement RNSeam.<Target> __props kids`.
   A new event RN nests under `nativeEvent` with no existing wrapper = §4.11 trigger A.
6. **Build `Web Debug` + defeat the Fable false-green:**
   ...
   ```
   Red after 2 attempts on something not in §4.10 = STOP (§4.11 trigger D).
7. **Verify WEB.** `cd AppEggShellGallery && ../eggshell dev-web` (restart after any LibClient change);
   `npm run observe -- snapshot -p web` and `--orientation landscape`. Renders, no console/health errors,
   correct in both orientations.
8. **Verify iOS.** One `eggshell dev-native` + one Metro serves both platforms.
   `npx react-native run-ios --simulator "iPhone 16" --no-packager`; relaunch the app;
   `xcrun simctl io booted screenshot`; read `xcrun simctl spawn booted log show --last 2m`. Repeat
   rotated. No red box, clean log.
9. **Verify ANDROID.** `adb reverse tcp:8081 tcp:8081`; force-stop + monkey-launch; `screencap`/`pull`;
   `adb logcat -d | grep -iE "Uncaught|Error|Warning|unique .key.|Color is expected"`. If the change is
   absent, confirm the patch reached the bundle:
   `curl -s "http://localhost:8081/index.bundle?platform=android&dev=true" | grep -c <marker>` (0 ⇒ restart
   Metro `--reset-cache`). Repeat rotated.
10. **Update the gallery page** (CLAUDE.md rule 10): the component's `AppEggShellGallery/src/Components/
    Content/` page must use the new F# API in pure F# (no render DSL). Add one if missing. Re-run step 7.
11. **Update `LEARNINGS.md`** (dated, newest at top): symptom → cause → fix → files for anything
    corrected. Update the §4.13 progress-table row.

**A11y + automation are baked into the primitive, not patched per call.** New shared `RNSeam` helpers do
this once: `assignA11yAndAutomation` (= `assignAccessibility` + auto-`testID` from label/slug +
`UiActionLog` register), and `assignAccessibility` extended to emit W3C `role` + `aria-*`. A ported
primitive that ships without name+role+state or without a `testId`/`UiActionLog` registration is NOT done
(CLAUDE.md rule 12).

### 4.10 Phase 4 pitfall checklist (symptom → cause → fix; consult BEFORE debugging from scratch)

If your symptom is here, apply the fix; do not re-derive. If it is not here and not obvious in one
attempt, STOP (§4.11). This is a Phase-4 superset of `DEV_RUNBOOK.md` §9.

| Symptom | Cause | Fix |
|---|---|---|
| `onLayout`/measurement width/height 0; drawer "always open"; responsive collapses | RNW delivers `{nativeEvent:{layout}}`; ReactXP flattened it; raw forwarding drops it | `RNSeam.assignOnLayout __props onLayout` (LEARNINGS 2026-06-30) |
...
| Header view ~2em; children inherit big font | `Header→"header"`→RNW `role="heading"`→`<h1>` | map `Header → "banner"` (RNW `<header>` landmark) |
| `lineHeight` huge (e.g. 384px) | numeric `lineHeight` read as a unitless CSS multiplier | route through `RNSeam.createTextStyle` (appends `px`) |
...
| Pure-F# `[<Component>]` ignores its theme (wrong color/size) | pure-F# components don't call `ComponentRegistry.GetStyles` | bake themed values into `makeViewStyles`/`makeTextStyles` |
| Absolute overlay/drawer wrapper swallows clicks+wheel beneath (invisible in screenshot) | wide/tall absolute wrapper stays a pointer target | `blockPointerEvents = true` (→ RN `box-none`); park closed overlay fully off-screen; locate via `elementFromPoint` |
| Build "green" but edit ignored | Fable cache false-green ("Skipped compilation … up-to-date", exit 0) | `touch` `.fs` / `rm -rf .build/<plat>/fable`; confirm `Started Fable compilation` + no `error FS` |
...
### 4.11 Phase 4 STOP-and-escalate triggers (hand back to a strong model; do not guess)

- **A. Seam design.** A prop/event whose RN shape has no existing `RNSeam` adapter and you would design
  one. Report: primitive, full F# signature, the prop, and the ReactXP-vs-RN shape difference.
- **B. Worklet / SIGABRT.** `Fatal signal 6 (SIGABRT)` / `getHostFunction`, or anything needing a
  UI-thread worklet. Fable worklets do not run on the UI thread (settled §4.6). `Animatable*` +
  `GestureView` are pre-flagged here. Report the animation/gesture intent + crash log.
- **C. Fable AST / plugin.** `Meta/FablePlugins` or any transpile/AST/plugin error (not a plain `error FS`
  in your code). Do not edit the AST. Report the full Fable error + file.
- **D. Build red after 2 attempts** on something not in §4.10. Report exact
  `error FS…`/red-box/logcat text, file, and the 2 attempts tried.
- **E. Any new gesture/animation design question** (PanResponder→RNGH mapping, gesture-state fields,
  Reanimated/Moti layer, reduce-motion, a gesture lacking a non-gesture alternative).
- **F. No clean RN/RNW equivalent**, or a param/style feature that will not map and the fallback is
  unclear. Report the primitive + the §4.7 limitation it hits.
- **G. Environment breakage** (`dotnet` killed/137, emulator/sim won't boot, Metro dead after
  `--reset-cache` + clean). Report the failing command + output.

### 4.12 Phase 4 command crib (full detail: `DEV_RUNBOOK.md` §3–§8 is source of truth)
...
cd AppEggShellGallery && ../eggshell dev-web    # :9080 ; open http://127.0.0.1:9080
...
### 4.13 Phase 4 sequencing, scope, and progress table

**Honest scope of "@chaldal removal."** Porting every primitive removes only the **four peripheral
packages** (`@chaldal/reactxp-virtuallistview`, `-webview`, `-netinfo`, `-imagesvg`) and deletes
`ReactXPBindings.fs`. The `@chaldal/reactxp` **core** (`ReactXPRaw = import "*"`) is still bound by
**Popup** (ContextMenus, Popup, PickerInternals/Base), **Storage** (LocalStorageService), **Location**
(With/Geolocation), **Styles.create\*** (Styles/Legacy/Types, FSharpDialect, Designtime, LibRouter/App),
the **higher-level Text/UiText/Button** (Components/Text, Components/UiText, Pressable.fs), and
`restart.setMainView`. Full core removal + deleting `vendor/reactxp-native-common` + the `postinstall`
hook is a **final gated batch** done only after the workstreams below land.

**Workstream A — remaining primitives (mechanical, §4.9 recipe).** Add the shared `RNSeam` helpers first
(no behavior change): promote `wrapOnScroll` from `ScrollView.fs` to `RNSeam`; add
`assignA11yAndAutomation`; extend `assignAccessibility` to emit `role` + `aria-*`; add imports for
Picker/FlatList/WebView/Linking/Dimensions/Keyboard/Clipboard/AccessibilityInfo. Then, low-risk first
(∥ = independent):
1. **UiText** → pure-F# `UiText.fs` on `RNSeam.Text` (retires the last `.typext.fs` in the seam).
2. **Platform APIs** (alone, broad blast radius): port `Runtime.platform`,
   `UserInterface` (RN `Dimensions`/`PixelRatio`/`Keyboard`), `Linking`, `Clipboard`
   (`@react-native-clipboard/clipboard`) into `RNSeam`/`RNPlatform`, keep module names + signatures,
   delete `ReactXPBindings.fs`. Port `AccessibilityAnnounce.fs` + `With/Accessibility.fs` to RN
   `AccessibilityInfo` (unblocks reduce-motion / screen-reader flags).
3. ∥ **NetInfo** web path → `@react-native-community/netinfo` (remove `-netinfo`).
4. ∥ **SVG** → `react-native-svg` (`ImageSvg`→`Svg`, `SvgPath`→`Path`, `fillColor`→`fill`; keep names so
   `Icons.fs` is unchanged) (remove `-imagesvg`).
5. ∥ **Link** (zero live call sites) on `RNSeam.Text` + `RNSeam.Pressable`; bake `role="link"` (real
   `<a>` on web), auto-testID, `UiActionLog` `Navigate`.
6. ∥ **Picker** (zero live call sites) on `@react-native-picker/picker` (native `<select>` on web); bake
   `role="combobox"`, auto-testID, record `InputCommit`. Web build not resolving under the alias =
   trigger A (fall back to View+Popup).
7. **WebView** (`#if WEB` `<iframe>` else `react-native-webview`; adapt `onMessage`/`onNavigationState
   Change`; wrap the `WebViewMethods` ref; verify `ThirdParty/ReCaptcha` still builds) (remove `-webview`).
8. **VirtualListView → FlatList** (highest risk, do last; live consumers `LC.VirtualListView` +
   `PaginatedVirtualListView`). Keep the full surface; re-map inside the seam (`itemList`→`data`,
   `renderItem`→`renderItem({item})`, `key`→`keyExtractor`, constant heights→`getItemLayout`,
   `scrollToTop(animate,top)`→`scrollToOffset`). **Gotcha:** ReactXP `onScroll` yields `(top,left)`; the
   promoted `wrapOnScroll` must yield `(y,x)` to match. Bake per-cell `role="listitem"` + auto-testID
   (`A11ySlug.testId "vlv-item" item.key`) + `UiActionLog` register/unregister (remove
   `-virtuallistview`).

After 1–8 are green, remove the four peripheral `@chaldal/reactxp-*` deps in one commit.

**Workstream B — animation + gestures (declarative Moti + RNGH; strong model; §4.6 worklet rule).** Only
four framework callers: `Carousel.fs`, `Draggable/Draggable.fs`, `Scrim.fs`,
`SegmentedControl/SegmentedControl.fs`. Public `LC.*` APIs stay identical. Pivot: imperative
`AnimatedValue`/`Animation.Timing().Start()` → declarative Moti `from`/`animate`/`transition`, split by
interaction: **state-driven transitions** (Scrim fade, Carousel/SegmentedControl settle) use Moti
`animate`; **finger-following** (SegmentedControl thumb drag, Draggable 2-axis) use a framework-owned JS
worklet shim `LibClient/src/ThirdParty/worklets.js` (Reanimated `useSharedValue`/`useAnimatedStyle` +
RNGH gesture worklets; `runOnJS` is safe there because the worklet is JS-authored). Steps: (0) add deps +
`react-native-worklets/plugin` (last) + `worklets.js` + `GestureHandlerRootView` at the AppShell root;
(1) new `Motion.fs` declarative surface with reduce-motion baked in (reads `LC.With.ReducedMotion`, swaps
to a zero-duration `NoAnimation` → snap); (2) port `RX.GestureView` to RNGH `Gesture.Pan()` +
`GestureDetector` with `.runOnJS(true)` + `RNSeam.adaptPanGestureState`, keep the signature, add the
non-gesture a11y fallback; (3–6) migrate Scrim, Carousel, SegmentedControl, Draggable; (7) delete the
dead layer (`Styles/Animation.fs`, animated rules in `New/RulesBasic.fs`, `makeAnimatable*Styles` +
`Animatable*Styles` in `FSharpDialect.fs`, `RNSeam.createAnimated*Style`; this clears the FSharpDialect
TODO); (8) pure-F# gallery pages + LEARNINGS entry.

**Workstream C — new RNW a11y/automation unlocks (bake into ported primitives NOW):** W3C `role`+`aria-*`
mapped once in `RNSeam.assignAccessibility`; `:focus-visible` + keyboard activation free via host
elements (Link→`<a>`, Picker→`<select>`); auto-`testID` + `UiActionLog` via `assignA11yAndAutomation`;
FlatList per-cell `listitem` + focus order; `prefers-reduced-motion`/`prefers-contrast`/`forced-colors`
flags enabled by the `With/Accessibility.fs`→`AccessibilityInfo` port. **Deferred, tagged, never faked:**
landmarks/heading-levels precision + skip links (`[rnw-blocked]`); `<dialog>`/`inert`/Popover (needs the
Popup port — core batch); React 19 form Actions (repo is React 18.3.1); Fabric synchronous focus (New-Arch
not adopted).

**Progress table** (update after each component; mirror into `LEARNINGS.md`). Already ported through
`RNSeam`: View, Text, Button→Pressable, Image, TextInput, ScrollView, ActivityIndicator.
...
A component is **done** only when web + iOS + Android (portrait + landscape — memory: AppTodo verify
matrix) are green AND the gallery page + LEARNINGS row are updated. Debug with Tier 1 (adb/simctl/
DevTools); gate with Tier 2 (`npm run observe`).
...
- ANY part of the Phase 4 seam design (pattern-setting, style DSL, animation layer, a primitive with no
  clean equivalent). See **§4.11** for the full Phase-4 STOP-and-escalate trigger list (A–G).
