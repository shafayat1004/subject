...
Executing goals B–E (see `modernization/goals-and-roadmap.md`; status dashboard in
`modernization/index.md`; live numbers via `scripts/status-dashboard.sh`):
- **A — DONE.** Render DSL is retired: zero `.render`/`.typext.fs`/`_autogenerated_` in framework
  libs (2 stragglers: `LibRouter/src/Components/{Link,Router}/*.typext.fs`). All components are
  pure-F# `[<Component>]`.
...
1. **Maintain the Engineering Log** (`knowledge-base/engineering-log.md`). Whenever I discover something
   ...
   **Codemem trust policy.** Act on codemem entries directly (that is the token savings); verify
   against on-disk state ONLY after the action fails. When you discover a stale entry in practice,
   repair/supersede it in the SAME session (`memory_forget` + rewrite via the skill). Entries store
   pointers + commands, not state snapshots; volatile status gets confidence 0.5 + a date.
...
6. **(Legacy, complete)** Goal A is done. If you hit a straggler `.render`/`.typext.fs`, convert to
   pure F# per `modernization/render-dsl-retirement.md`; never patch or copy autogenerated Render.
...
- Modern pure-F# component = single `Foo.fs` using `type LC with [<Component>] static member`
  (see `Tabs.fs`, `Heading.fs`, `HandheldListItem.fs`). Public types live in the same file (often
  under `module LC = module Foo`, or `namespace ... module Foo` for nested paths like `Nav.Top.Item`).
...
- `.fsproj` compile order matters: a component's source files precede any generated files.
...
`gallery-page-add`, `release-build`, `verify-feature`, `screenshot-describe`, `codemem-update`,
`spike-driven`, `mssql-debug`, `orleans-test-triage`, `symptom-matcher`, `scaffolding-validator`,
`impeccable`. Prefer invoking the matching skill over improvising commands.

## Deterministic gates (enforcement lives outside the prompt)

Prose rules get violated; scripts do not. These gates are installed and must stay green:
- `scripts/forbidden-path-guard.sh` — blocks edits to `_autogenerated_`/`*.render`/`*.typext.fs`/
  stray `.fs.js`; warns on `App*`/`Suite*`. Wired as a **PreToolUse hook** (`.claude/settings.json`)
  and a **git pre-commit hook** (reinstall after fresh clone: `scripts/install-git-hooks.sh`).
  A second PreToolUse hook blocks direct `dotnet fable` invocations.
- `scripts/verify-done.sh <build-log>` — the done-gate: fails on `error FS`/"Skipped compilation"
  false greens. **No "done" claim without a verify-done PASS or equivalent pasted evidence.**
- `scripts/ci-gate.sh <files...>` — one-shot: path guard + `eggshell-fmt --check` + style-leak +
  a11y scans. Run on changed files before committing UI work; destined to be the CI entry.
- `scripts/symptom-match.sh "<error>"` — greps runbooks/troubleshooting for known symptoms BEFORE
  improvising root-cause analysis (skill: `symptom-matcher`).
- `scripts/status-dashboard.sh` — generated modernization numbers; use instead of trusting
  hand-maintained status pages.

## Build & validate

(Commands confirmed during baseline — update here + in `knowledge-base/engineering-log.md` as learned.)
- Frontend lib type-check / build: `./eggshell build-lib` from the lib dir (Fable). Backend libs:
  `dotnet build`.
