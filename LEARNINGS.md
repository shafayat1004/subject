...
**Phase 2b SignalR server (2026-06-28):** Vendored `Fable.SignalR.AspNetCore` into `Meta/LibSignalRServer/` (sources from Shmew/Fable.SignalR GitHub + EggShell's private 0.14.0 `CancellationToken` streamFrom extension). `LibLifeCycleHost` → `ProjectReference` instead of NuGet `Fable.SignalR.AspNetCore` 0.14.0. Server stack: stock `Microsoft.AspNetCore.App` + `Microsoft.AspNetCore.SignalR.Protocols.NewtonsoftJson` + `Fable.Remoting.Json` + `TaskBuilder.fs`; server-only shared types in `Meta/LibSignalRServer/src/Shared.fs` (no `Fable.Core`; client keeps its own `Shared.fs` with `[<Erase>]` HubMethod). Pin `Microsoft.IO.RecyclableMemoryStream` 2.3.2 (3.x breaks `AggressiveBufferReturn` ctor arg in vendored `MsgPack.fs`). EggShell real-time handlers unchanged (`/realTime`, `/api/v1/realTime`, typed DU wire contract). Validated: `dotnet build Meta/LibSignalRServer`, `dotnet build LibLifeCycleHost`. End-to-end push still needs gallery + backend runtime (spike reference: `../eggshell-rnw-spike/signalr-server/` for stock ASP.NET Core shape; typed hub path is what EggShell uses).

**Phase 2a SignalR client (2026-06-28):** Vendored Fable.SignalR 0.16.0 sources into `Meta/LibSignalRClient/` (Fable.Core 5, `@microsoft/signalr` in `LibClient/package.json`). `LibUiSubject` → `ProjectReference` instead of NuGet `Fable.SignalR`. Renamed streaming `Subject<'T>` → `StreamSubject<'T>` so lifecycle `Subject<'Id>` is not shadowed. LibUiSubject files use `#if !FABLE_COMPILER` around `open LibLifeCycleTypes.SubjectTypes` (Fable path relies on auto-opened `LibLifeCycleTypes_SubjectTypes`). `LibSignalRClient.fsproj` omits `FABLE_COMPILER` so `dotnet build` typechecks; Fable injects `FABLE_COMPILER` at transpile time (`inline` on `HubConnectionBuilder.build` / `SignalR.connect`). Validated: `dotnet build LibUiSubject`, `dotnet fable LibUiSubject.fsproj` → `RealTimeService.js`.
