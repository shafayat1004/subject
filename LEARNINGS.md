...
**Fable.SignalR.Subject shadows lifecycle `Subject`** in `With/Subjects*.fs` constraint lists — add `open LibLifeCycleTypes.SubjectTypes` after other opens (same workaround as `SubjectService.fs`). Do **not** use `open LibLifeCycleTypes_SubjectTypes`: that module name exists only when `LibLifeCycleTypes` itself is compiled with `FABLE_COMPILER`; `dotnet build` references the DLL where the module is `LibLifeCycleTypes.SubjectTypes`.
