...
type StreamResult<'T> =
    ...
    member inline this.subscribe (subscriber: IStreamSubscriber<'T>, ?logger: string -> unit) =
        this.subscribe'(subscriber)
        |> fun sub -> { new System.IDisposable with
            member _.Dispose () =
                logger
                |> Option.iter (fun l ->
                    l "Disposing of subscription"
                )
                sub.dispose()
        }
    ...
    /// available from the stream.
    member inline this.subscribe (subscriber: StreamSubscriber<'T>, ?logger: string -> unit) =
        this.subscribe(unbox<IStreamSubscriber<'T>> subscriber, ?logger = logger)
