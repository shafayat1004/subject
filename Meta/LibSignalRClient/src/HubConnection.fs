...
/// Stream implementation to stream items to the server (named StreamSubject to avoid clobbering LibLifeCycleTypes.Subject).
type StreamSubject<'T> =
...
module Bindings =
    ...
    type SignalR =
        ...
        [<Emit("new $0.Subject()")>]
        member _.StreamSubject<'T> () : StreamSubject<'T> = jsNative
...
[<RequireQualifiedAccess>]
type HubMailbox<'ClientApi,'ServerApi> =
    | ProcessSends
    | Send of callback:(unit -> Async<unit>) * maybeLogCallback: Option<string -> unit>
...
type HubConnection<'ClientApi,'ClientStreamFromApi,'ClientStreamToApi,'ServerApi,'ServerStreamApi>
    (hub: IHubConnection<'ClientApi,'ClientStreamFromApi,'ClientStreamToApi,'ServerApi,'ServerStreamApi>, handlers: Handlers) =
            ...
            let rec loop (waitingInvocations: Map<System.Guid,AsyncReplyChannel<'ServerApi>>) (waitingConnections: (unit -> Async<unit>) list) =
                async {
                    ...
                    return!
                        ...
                        | HubMailbox.Send (action, maybeLogCallback) ->
                            let newConnections, msg =
                                if hub.state = ConnectionState.Connected then
                                    action() |> fun a -> Async.StartImmediate(a, cts.Token)
                                    [], "Already connected so started immediately"
                                else
                                    [ action ], "Not yet connected, so deferred action"

                            //maybeLogCallback
                            //|> Option.iter (fun logCallback -> logCallback msg)
    ...
    member _.send (msg: 'ClientApi) =
        async { return mailbox.Post(HubMailbox.Send((fun () -> hub.send(msg)), None)) }
    ...
    /// Streams from the hub, making an immediate subscriber to minimize race conditions whereby messages can be missed between the streamFrom and
    /// subscribe calls.
    ///
    /// Note: I still very occasionally see a dropped message, which I cannot account for. However, ensuring the streamFrom and subscribe calls occur
    /// in quick succession in a synchronous manner has reduced the incidence to near zero.
    member _.stream (msg: 'ClientStreamFromApi) (subscriber: StreamSubscriber<'ServerStreamApi>) (logger: string -> unit) =
        mailbox.PostAndAsyncReply <| fun reply ->
            HubMailbox.Send(
                (fun () ->
                    async {
                        logger $"[{msg}] subscribing"
                        let subscription = hub.streamFrom(msg).subscribe(subscriber, logger = fun message -> logger $"[{msg}] {message}")
                        return reply.Reply(subscription)
                    }),
                Some logger
            )

    /// Streams from the hub.
    [<Obsolete("Use stream to subscribe at the same time and mitigate race conditions")>]
    member _.streamFrom (msg: 'ClientStreamFromApi) =
        mailbox.PostAndAsyncReply <| fun reply ->
            HubMailbox.Send((fun () -> async { return reply.Reply(hub.streamFrom(msg)) }), None)
    ...
    member _.streamTo (subject: ISubject<'ClientStreamToApi>) =
        async { return mailbox.Post(HubMailbox.Send((fun () -> hub.streamTo(subject)), None)) }
