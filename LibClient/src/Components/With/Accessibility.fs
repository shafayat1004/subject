...
module LC =
    module With =
        module Accessibility =
            let private tryScreenReaderEnabled () : bool =
                try
                    !!ReactXPRaw?Accessibility?isScreenReaderEnabled()
                with _ ->
                    false
            ...
            let private tryAsyncBool (query: string) (onResult: bool -> unit) : unit =
                async {
                    try
                        let! result = ReactXPRaw?Accessibility?(query)() |> Async.AwaitPromise |> Async.TryCatch
                        match result with
                        | Ok v -> onResult v
            ...
            let private tryFontScale (onResult: float -> unit) : unit =
                try
                    let scale = !!ReactXPRaw?PixelRatio?getFontScale()
                    onResult scale
                with _ ->
                    onResult 1.0
            ...
            let private querySettings () : AccessibilitySettings =
                {
                    AccessibilitySettings.defaults with
                        ScreenReaderEnabled = tryScreenReaderEnabled ()
                }
            ...
            let private trySubscribeBoolEvent
                ...
                : unit =
                try
                    let handler (isEnabled: bool) =
                        bump (fun s -> update isEnabled s)

                    let subscription = ReactXPRaw?Accessibility?addEventListener(eventName, handler)
                    ...
                    unsubscribers.Add(fun () ->
                        ...
                        with _ ->
                            try
                                ReactXPRaw?Accessibility?removeEventListener(eventName, handler) |> ignore
                            with _ ->
                                ())
                with _ ->
                    ()
            ...
            let private subscribe (onChange: AccessibilitySettings -> unit) : (unit -> unit) =
                ...
                trySubscribeBoolEvent "screenReaderChanged" (fun v s -> { s with ScreenReaderEnabled = v }) bump unsubscribers
                ...
                trySubscribeBoolEvent "reduceTransparencyChanged" (fun v s -> { s with ReduceTransparency = v }) bump unsubscribers
                ...
                tryAsyncBool "isReduceMotionEnabled" (fun v ->
                    bump (fun s -> { s with ReduceMotion = v }))

                tryAsyncBool "isBoldTextEnabled" (fun v ->
                    bump (fun s -> { s with BoldText = v }))

                tryAsyncBool "isReduceTransparencyEnabled" (fun v ->
                    bump (fun s -> { s with ReduceTransparency = v }))

                tryAsyncBool "isInvertColorsEnabled" (fun v ->
                    bump (fun s -> { s with InvertColors = v }))

                tryAsyncBool "isGrayscaleEnabled" (fun v ->
                    bump (fun s -> { s with Grayscale = v }))

                tryFontScale (fun scale ->
                    bump (fun s -> { s with FontScale = scale }))
