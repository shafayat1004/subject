...
module private Styles =
    ...
    let errorContainer = makeViewStyles { padding 10 }
...
type private Helpers =
    ...
    static member _PaginatedVirtualListView
        ...
        let nextItems: IStateHook<seq<PaginatedVirtualListItems> option> =
            Hooks.useState None

        let nextPage: IStateHook<uint64> = Hooks.useState 0UL

        let dataFetching: IStateHook<DataFetchingState> =
            Hooks.useState DataFetchingState.Loading
        ...
        let appendItems
            ...
            (source: PaginatedVirtualListItems seq)
            : PaginatedVirtualListItems seq =
            source |> Seq.append currentItems |> Seq.distinctBy (fun item -> item.getKey)

        let removeItems (key: string) (source: PaginatedVirtualListItems seq) : PaginatedVirtualListItems seq =
            source
            |> Seq.filterMap (fun item -> if key = item.getKey then None else Some item)
        ...
        let storeData (data: seq<PaginatedVirtualListObject>) =
            let updatedData = data |> Seq.map PaginatedVirtualListItems.CustomListItem

            nextItems.update (Some updatedData)
            updatedData |> Seq.append allItems.current |> allItems.update

        let getAndUpdateItems (currentPage: uint64) =
            async {
        ...
        let loadNextPage (page: uint64) =
            async {
                match nextItems.current with
                | Some value ->
                    nextItems.update None
                    value |> appendItems allItems.current |> removeItems "Loader" |> allItems.update
                | None ->
                    match dataFetching.current with
                    | Loading
                    | Completed
                    | FetchingFailed -> do! getAndUpdateItems (page)
                    | _ -> Nothing
            }

        Hooks.useEffect ((fun () -> loadNextPage (nextPage.current) |> startSafely), [||])
        ...
        Hooks.useEffect (
            (fun () ->
                ...
                | true -> Nothing
                | false ->
                    nextLoadHeight.update (
                        (allItems.current
                         |> Seq.map (fun item -> item.KeyAndMaybeHeight |> snd |> Option.defaultValue 1)
                         |> Seq.sum)
                        - minLoadThreshold
                    )
        ...
        let render (item: PaginatedVirtualListItems) : ReactElement =
            match item with
            | PaginatedVirtualListItems.CustomListItem item -> renderItem item
            ...
            | PaginatedVirtualListItems.NoItem ->
                LC.UiText(styles = [| Styles.noItems |], children = [| LC.Text "No Items" |])
             ...
             |> appendItems (
                 (match dataFetching.current with
                  ...
                  | DataFetchingState.FetchingFailed ->
                      seq {
                          PaginatedVirtualListItems.DataFetchingError(
                              "Apologies, there is an internet issue. Please try again.",
                              (fun _ -> loadNextPage nextPage.current)
                          )
                      }
        ...
        let vlvItems: seq<VirtualListItem<PaginatedVirtualListItems>> =
            paginatedVirtualListItems
            |> Seq.map (fun item ->
        ...
        element {
            LC.VirtualListView(
                ...
                scrollSideEffect =
                    (fun (top, _) ->
                        if
                            top > nextLoadHeight.current
                            && not hasEndReached.current
                            && dataFetching.current = DataFetchingState.Completed
                        then
                            loadNextPage nextPage.current |> startSafely),
                ?styles = styles
            )
        }
...
type LibClient.Components.Constructors.LC with
    ...
    static member PaginatedVirtualListView
        ...
        element {
            let dataMapper =
                (fun (items: Async<AsyncData<PaginatedVirtualListItem<'Item> seq>>) ->
                    items
