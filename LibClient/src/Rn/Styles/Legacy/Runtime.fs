...
module Runtime =
    ...
    | StaticRules        of RnStyleRulesObject
    ...
    let prepareStylesForPassingToRnComponent<'T> (fullyQualifiedComponentName: string) (styles: List<RuntimeStyles>) : 'T =
        match fullyQualifiedComponentName with
        | "Rn.Components.AniView"
        | "Rn.Components.AniText"
        | "Rn.Components.AniTextInput"
        | "Rn.Components.AniImage" ->
            ...
            |> List.filterMap (function
                ...
                | RuntimeStyles.Sheet _ ->
                    Log.Error("Found RuntimeStyles.Sheet passed to an animated base Rn component, which is meaningless. Exception: {exception}", exceptionForStackTrace)
                    None
                ...
                | RuntimeStyles.StaticRules lazyValue ->
                    AnimatedComponentPropStyle.StaticRules (lazyValue.CreateForRnComponent fullyQualifiedComponentName) |> Some
            )
            :> obj :?> 'T
        ...
        | _ ->
            ...
            |> List.filterMap (fun currStyles ->
                ...
                | RuntimeStyles.AnimatedAnimations _ ->
                    Log.Error("Found RuntimeStyles.{currStyles} passed to a base Rn component. Use animated Rn components instead. Exception: {exception}", (unionCaseName currStyles), exceptionForStackTrace)
                    None
                | RuntimeStyles.None -> None
                | RuntimeStyles.StaticRules lazyValue ->
                    Some (lazyValue.CreateForRnComponent fullyQualifiedComponentName)
            )
            |> Array.ofList
            :> obj :?> 'T
    ...
    let extractRnStyleValue (key: string) (rnStyles: array<obj>) : Option<obj> =
        rnStyles
        |> Seq.rev
    ...
    module AnimationManager =
        ...
        type InstanceAnimationDataStore = {
            ...
            Rules:      Map<ClassName, RnStyleRulesObject>
        ...
        let private getOrCreateRules (instanceId: System.Guid) (className: string) (constructor: AnimatedRulesConstructor) : RnStyleRulesObject =
