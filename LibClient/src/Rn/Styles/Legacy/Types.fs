...
type TypedRnStyleRule = RawRnStyleRule * StyleRuleType

type LazilyCreatedRnStyleObject (typedValues: seq<TypedRnStyleRule>) =
    // NOTE since we do our own caching of styles, by associating them with class names,
    // we completely disable caching on the Rn side. This also avoids the annoying
    ...
    let shouldCacheRnInternal = false
    ...
    static let fullyQualifiedComponentNameToAllowedTypes = Map.ofList [
        ("Rn.Components.Text",      Set.ofList [StyleRuleType.Text; StyleRuleType.Flexbox; StyleRuleType.View; StyleRuleType.Transform])
        ("Rn.Components.UiText",    Set.ofList [StyleRuleType.Text; StyleRuleType.Flexbox; StyleRuleType.View; StyleRuleType.Transform])
        ("Rn.Components.TextInput", Set.ofList [StyleRuleType.Text; StyleRuleType.Flexbox; StyleRuleType.View; StyleRuleType.Transform])
    ]
    static let allowedTypesFallback = Set.ofList [StyleRuleType.Flexbox; StyleRuleType.View; StyleRuleType.Transform]

    let mutable cache: Map<(* fullyQualifiedComponentName *) string, RnStyleRulesObject> = Map.empty

    member this.GetRawStyleRules : seq<RawRnStyleRule> =
        typedValues
        |> Seq.map fst
    ...
    member private _.ReallyCreateForRnComponentAndCache (fullyQualifiedComponentName: string) : RnStyleRulesObject =
        ...
        let result: RnStyleRulesObject =
            match fullyQualifiedComponentName with
            | "Rn.Components.Text"
            | "Rn.Components.UiText" ->
                Rn.RnPrimitives.createTextStyle(createObj typelessAllowedRules)
            | _ -> Rn.RnPrimitives.createViewStyle(createObj typelessAllowedRules)
...
| StaticRules        of LazilyCreatedRnStyleObject
